...
void UpdateData::UpdatePatientMenu(string name)
{
	...
	string searchUpd_query = "SELECT * FROM patient WHERE Patient_ID = '" + to_string(Patient_ID) + "';";
	...
	if (!qstate)
	{
		...
		change:
			...
			if (UpdChoice == "1")
			{
				...
				string update_query = "UPDATE patient SET Patient_Name = '" + Patient_Name + "' WHERE Patient_ID = '" + to_string(Patient_ID) + "';";
			...
			else if (UpdChoice == "2")
			{
				...
				string update_query = "UPDATE patient SET Patient_Gender = '" + Patient_Gender + "'WHERE Patient_ID = '" + to_string(Patient_ID) + "';";
				const char* q = update_query.c_str();
				qstate = mysql_query(conn, q);
				cout << "\n Successfully Updated!" << endl;
			...
			else if (UpdChoice == "4")
			{
				...
				string update_query = "UPDATE patient SET  Patient_Address = '" + Patient_Address + "'WHERE Patient_ID = '" + to_string(Patient_ID) + "';";
				const char* q = update_query.c_str();
				qstate = mysql_query(conn, q);
				cout << "\n Successfully Updated!" << endl;
				cout << "Do you want to continue update record? [Y/N]: ";
			...
			else if (UpdChoice == "5")
			{
				...
				string update_query = "UPDATE patient SET Patient_Height = '" + to_string(Patient_Height) + "'WHERE Patient_ID = '" + to_string(Patient_ID) + "';";
				const char* q = update_query.c_str();
				qstate = mysql_query(conn, q);
				cout << "\n Successfully Updated!" << endl;
				cout << "Do you want to continue update record? [Y/N]: ";
			...
			else if (UpdChoice == "6")
			{
				...
				string update_query = "UPDATE patient SET Patient_Weight = '" + to_string(Patient_Weight) + "'WHERE Patient_ID = '" + to_string(Patient_ID) + "';";
				const char* q = update_query.c_str();
				qstate = mysql_query(conn, q);
				cout << "\n Successfully Updated!" << endl;
				cout << "Do you want to continue update record? [Y/N]: ";
			...
			else if (UpdChoice == "7")
			{
				...
				string update_query = "UPDATE patient SET Patient_TelNo = '" + Patient_TelNo + "'WHERE Patient_ID = '" + to_string(Patient_ID) + "';";
				const char* q = update_query.c_str();
				qstate = mysql_query(conn, q);
				cout << "\n Successfully Updated!" << endl;
				cout << "Do you want to continue update record? [Y/N]: ";
			...
			else if (UpdChoice == "8")
			{
				...
				string update_query = "UPDATE patient SET Patient_Email = '" + Patient_Email + "'WHERE Patient_ID = '" + to_string(Patient_ID) + "';";
				const char* q = update_query.c_str();
				qstate = mysql_query(conn, q);
				cout << "\n Successfully Updated!" << endl;
				cout << "Do you want to continue update record? [Y/N]: ";
			...
			else if (UpdChoice == "9")
			{
				...
				string update_query = "UPDATE patient SET Medical_History  = '" + Medical_History + "'WHERE Patient_ID = '" + to_string(Patient_ID) + "';";
				const char* q = update_query.c_str();
				qstate = mysql_query(conn, q);
				cout << "\n Successfully Updated!" << endl;
				cout << "Do you want to continue update record? [Y/N]: ";
			...
			else if (UpdChoice == "10")
			{
				...
				string update_query = "UPDATE patient SET Diagnosed_Symptoms  = '" + Diagnosed_Symptoms + "'WHERE Patient_ID = '" + to_string(Patient_ID) + "';";
				const char* q = update_query.c_str();
				qstate = mysql_query(conn, q);
				cout << "\n Successfully Updated!" << endl;
				cout << "Do you want to continue update record? [Y/N]: ";
			...
			else if (UpdChoice == "11")
			{
				while (true)
				{
					int choice; // Use an integer for the user input
					cout << "Enter Active Status (1 for Active, 2 for Inactive): ";
					cin >> choice;

					// Map numeric input to corresponding strings
					if (choice == 1)
					{
				...
				string update_query = "UPDATE patient SET Active_Status = '" + Active_Status + "' WHERE Patient_ID = '" + to_string(Patient_ID) + "';";
				const char* q = update_query.c_str();
				qstate = mysql_query(conn, q);
				cout << "\n Successfully Updated!" << endl;
				cout << "Do you want to continue update record? [Y/N]: ";
...
void  UpdateData::UpdatePatientOwnDetail(int PatientID, string name)
{
	...
	SetConsoleColor(0, 9);
	cout << "*************************************" << endl;
	cout << " UPDATE YOUR OWN  PATIENT'S DETAIL   " << endl;
	cout << "*************************************" << endl;
	SetConsoleColor(0, 11);
	...
	if (!qstate)
	{
		...
		if (confirmUpd == 'Y' || confirmUpd == 'y')
		{
			...
			cout << "\nYour choice (choose 1 number from 1-8)>> ";
			cin >> UpdChoice;
			...
			if (UpdChoice == "1")
			{
				...
				if (continueUpd == 'Y' || continueUpd == 'y')
					UpdatePatientOwnDetail(PatientID, name);
				else
					lg.PatientMainMenu(name, Patient_ID);
			}
	...
	if (!qstate)
	{
		...
		change2:
			...
			else if (UpdChoice == '5')
			{
				...
				{
					int choice; // Use an integer for the user input
					cout << "Enter 1 for 'Available' or 2 for 'Unavailable': ";
					cin >> choice;
					...
					{
						cout << "Invalid input. Please enter 1 for 'Available' or 2 for 'Unavailable'.\n";
					}
				}
...
void UpdateData::UpdateDrugMenu(string name)
{
	...
	cout << "\nHere's the record found: \n" << endl;
	string searchUpd_query = "SELECT * FROM medication WHERE Medication_ID = '" + to_string(Medication_ID) + "';";
	const char* q = searchUpd_query.c_str();
	qstate = mysql_query(conn, q) ;
	...
	if (!qstate)
	{
		...
		while (row = mysql_fetch_row(res))
		{
			...
			cout << "Medication_Type: " << row[2] << endl;
	...
	if (!qstate)
	{
		...
		if (confirmUpd == 'Y' || confirmUpd == 'y')
		{
			...
			else if (UpdChoice == "3")
			{
				...
				string update_query = "UPDATE medication_transaction SET Patient_ID = '" + to_string(Patient_ID) + "' WHERE Transaction_ID = '" + to_string(Transaction_ID) + "';";
			...
			else if (UpdChoice == "4")
			{
				...
				string update_query = "UPDATE medication_transaction SET Status = '" + status + "'WHERE Transaction_ID = '" + to_string(Transaction_ID) + "';";
				const char* q = update_query.c_str();
				qstate = mysql_query(conn, q);
			...
			else if (UpdChoice == "5")
			{
				...
				// Construct the query
				string update_query = "UPDATE medication_transaction SET Hospital_ID = " + to_string(Hospital_ID) +
					" WHERE Transaction_ID = " + to_string(Transaction_ID) + ";";
				...
				else
				{
					...
					cout << "Failed to update Hospital_ID. Error: " << mysql_error(conn) << endl;
				}

				// Ask to continue updating records
				cout << "Do you want to continue updating records? [Y/N]: ";
				cin >> continueUpd;
...
void UpdateData::UpdateStaff(string name)
{
	...
	string searchUpd_query = "SELECT * FROM Staff WHERE Staff_ID = '" + Staff_ID + "';";
	const char* q = searchUpd_query.c_str();
	qstate = mysql_query(conn, q);
	if (!qstate)
	{
		...
		if (confirmUpd == 'Y' || confirmUpd == 'y')
		{
			...
			cout << "[4] Staff Tel No" << endl;
			cout << "[5] Staff Position" << endl;
			cout << "[6] Active Status" << endl;
			cout << "[7] Hospital ID" << endl;
			...
			{
				// Prompt user for Hospital ID with validation
				bool valid = false;
				do
				{
				...
				cout << "Executing query: " << update_query << endl; // Debugging
				...
				if (qstate == 0) {
					...
					cout << "\n Update failed! Error: " << mysql_error(conn) << endl;
				}
